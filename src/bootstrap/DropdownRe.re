...
module Menu = {
  ...
  let make =
    ...
    let className =
      ...
      |> String.concat(" ");
    <div className tabIndex=(-1) ariaHidden={!isOpen} role="menu">
      children
    </div>;
  };
};
...
module Item = {
  ...
  let make =
    ...
    switch (href) {
    | None =>
      <button className tabIndex onClick=handleClick type_> children </button>
    | Some(href) =>
      <a className tabIndex onClick=handleClick type_ href> children </a>
    };
  };
};
